...
class RetryHandler:
    ...
    def pdf_generation(self, model_dir, task_id, messages, code_path):
        ...
        ensure_dir(f"{model_dir}/p{task_id}")
        input_path = f"{model_dir}/p{task_id}/p{task_id}_input.txt"
        output_path = f"{model_dir}/p{task_id}/p{task_id}_output.txt"
        write_file(input_path, prompt)
        write_file(output_path, answer)
        final_code_path = f"{model_dir}/p{task_id}/p{task_id}_final.py"
